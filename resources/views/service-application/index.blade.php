              ...
              <tbody>
                @if($applications && $applications->count())
                  ...
                  <tr>
                    ...
                    <td><a href="{{route('post.show',['job'=>$application->post->id])}}">{{substr($application->post->service_title,0,14)}}...</a></td>
                    <td>{{$application->created_at}}</td>
                    <td><a href="{{route('CarServiceApplication.show',['id'=>$application])}}" class="btn primary-outline-btn">View</a>
                      <form action="{{route('CarSaeviceApplication.destroy')}}" method="POST" class="d-inline-block">
                        @csrf
                        @method('delete')
